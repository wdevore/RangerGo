...
func (g *gameLayer) Build(world api.IWorld) {
	...
	g.trackerComp.Configure(targetSize, entityTriangle, entityStarShip|entityBoundary, &g.b2World)
	g.trackerComp.SetPosition(0.0, 0.0)

	g.starShipComp = NewStarShipComponent("StarShip", g)
	g.starShipComp.Configure(targetSize/2.0, entityStarShip, entityTriangle|entityBoundary, &g.b2World)
	g.starShipComp.Reset(0.0, 40.0)

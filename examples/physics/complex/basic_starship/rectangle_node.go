...
func (r *RectangleNode) Polygon() api.IPolygon {
	return r.polygon
}
...
func (r *RectangleNode) SetBounds(minx, miny, maxx, maxy float64) {
	mesh := r.polygon.Mesh()
	mesh.SetVertex(minx, miny, 0)
	mesh.SetVertex(minx, maxy, 1)
	mesh.SetVertex(maxx, maxy, 2)
	mesh.SetVertex(maxx, miny, 3)
	r.SetDirty(true)
}

...
// Configure component
func (t *TrackingComponent) Configure(scale float64, categoryBits, maskBits uint16, b2World *box2d.B2World) {
	t.scale = scale
...
func (t *TrackingComponent) SetPosition(x, y float64) {
...
func (t *TrackingComponent) Reset(x, y float64) {
	t.visual.SetPosition(x, y)
...
func (t *TrackingComponent) Update() {
	...
	ray := box2d.MakeB2Vec2(t.targetPosition.X()-bodyPos.X, t.targetPosition.Y()-bodyPos.Y)
...
func buildComp(t *TrackingComponent, b2World *box2d.B2World) {
	...
	for _, v := range verts {
		vertices = append(vertices, box2d.B2Vec2{X: v.X() * t.scale, Y: v.Y() * t.scale})
	}

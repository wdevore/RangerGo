...
func NewStarShipComponent(name string, parent api.INode) *StarShipComponent {
	...
	// Higher strength if Force used
	o.thrustStrength = 50.0
	// Lower strength if Impulse used
...
// BodyPosition gets the Hull's physics position
func (s *StarShipComponent) BodyPosition() (x, y float64) {
	return s.b2BodyHull.GetPosition().X, s.b2BodyHull.GetPosition().Y
}
...
func (s *StarShipComponent) ApplyImpulseThrust() {
	...
	s.b2BodyHull.ApplyForce(dir, s.b2BodyHull.GetWorldCenter(), true)
}
